...
`src/robot_supports/hardware/damiao` 是当前真机参考 Hardware Backend；它实现 Core 的 `MotorBus` 合同，读取 `hardware.yaml`，并以共享库形式导出 `create_motor_bus` / `destroy_motor_bus`
...
1. 准备独立急停和可靠机械支撑
2. 确认串口设备、全部 motor ID、master ID 和电机型号
3. 逐轴确认位置、速度和力矩反馈方向
...
./install/serial_arm_core/bin/serial_arm_terminal \
  ...
  --config src/robot_supports/robots/dm_arm/description/config/core/gray.yaml
```
...
`serial_arm/` 是 Stable Framework，包含 `serial_arm_core`、通用 ros2_control bringup，以及 planned / reserved 状态的 LeRobot、Isaac Sim 适配目录；新增具体机器人或 Hardware Backend 时原则上不修改这里

`robot_supports/` 是 Extension Area，包含 Hardware Backend、Robot Description、Robot Config、MoveIt Config 和 Robot Profile；新增机器人产生的变化应集中在这里
...
`dm_arm` 是 Robot Support；`gray` / `white` 是模型目录变体，`dm_arm_gray` / `dm_arm_white` 是运行 Profile Variant

### 6.3. serial_arm_ros2_control
...
| 机械臂 | 配置 | URDF |
|---|---|---|
| 白色带打印夹爪 | `src/robot_supports/robots/dm_arm/description/config/core/white.yaml` | `model/white/urdf/dm_arm.urdf` |
| 灰色无打印夹爪 | `src/robot_supports/robots/dm_arm/description/config/core/gray.yaml` | `model/gray/urdf/dm_arm_no_gripper.urdf` |

### 6.6. Robot Profile

Robot Profile 将一个可启动实例需要的 Core Config、Hardware Backend、Hardware Config、Description Model、ros2_control Xacro、Controller Config 和 MoveIt Config 组合在一起；Profile 名必须在工作空间内全局唯一，建议采用 `<robot_name>_<variant>`，例如 `dm_arm_gray`、`dm_arm_white`、`pieper_arm_default`

`package + relative path` 会解析为：

```text
<package share>/<relative path>
```
...
| 字段 | 说明 |
|---|---|
| `core.package` | Core YAML 所在 ROS package |
| `core.config` | Core 配置相对路径 |
| `hardware.plugin` | HardwareLoader 加载的 Backend 名称或共享库路径 |
| `hardware.config_package` | Hardware 实例配置所在 ROS package |
| `hardware.config` | Hardware Backend 配置相对路径 |
| `description.package` | Robot Description 所在 ROS package |
| `description.urdf` | display 使用的普通 URDF 路径 |
| `description.ros2_control_xacro` | 注入 Core 和 Hardware 参数的 ros2_control Xacro |
| `controllers.package` | controller_manager 配置所在 ROS package |
| `controllers.config` | ros2_controllers YAML 相对路径 |
| `moveit.package` | 当前 Profile 使用的 MoveIt 配置包 |

### 6.7. Hardware Backend Contract

SerialArm Core 定义合同，Hardware Backend 适配 Core；Backend 必须把厂商协议、厂商单位、电流或其他底层量转换为 SerialArm 的执行器侧语义
...
Backend 负责厂商协议转换、单位转换、current 与 torque 的换算、必要的执行器侧或 Backend 内部控制、厂商错误码处理和 `HardwareCapabilities` 构造；Core 不会为某个厂商降低控制语义，不增加 current command，也不认识厂商协议细节

### 6.8. Adding a Robot Support
...
1. 创建 `robot_supports/robots/<robot_name>/`
2. 建立 `description/config/`
3. 建立 `description/model/<variant>/urdf/` 和 `description/model/<variant>/meshes/`
4. 添加 Core YAML
5. 添加该 Robot 使用的 Hardware instance YAML
6. 添加 `ros2_controllers.yaml`
7. 可选添加 MoveIt Config
8. 在 `robot_supports/profiles/config/robot_profiles.yaml` 注册 `<robot_name>_<variant>`
9. 用 `display.launch.py` 验证模型
10. 用 `hardware.launch.py` 启动真实硬件或离线模式

新增 Robot Support 不应修改 `src/serial_arm/`；如果同一机械臂有多个实体 Variant，则在 `description/model/` 下增加多个 model 目录，并在 `description/config/` 中提供对应 Core YAML，再注册对应 Robot Profile

### 6.9. Adding a Hardware Backend
...
1. 创建 `robot_supports/hardware/<vendor>/`
2. 实现 `MotorBus`
3. `configure()` 解析自己的 Hardware YAML
4. `read()` 输出标准 `pos / vel / tor`
5. `write()` 接受标准 `pos / vel / tor / kp / kd`
6. Backend 自行完成 vendor protocol、unit conversion、current 与 torque 换算和必要的本地控制
7. `capabilities()` 返回 Core Safety 所需范围
8. 导出 `create_motor_bus` 和 `destroy_motor_bus`
9. 在 Robot Profile 中注册 `hardware.plugin`、`hardware.config_package` 和 `hardware.config`

新增 Backend 不应修改 `serial_arm/core`、`serial_arm/bringup/ros2_control`、`serial_arm/bringup/lerobot` 或 `serial_arm/bringup/isaac_sim`
...
### 8.3. 构建 Damiao Hardware Backend
...
```bash
colcon build --symlink-install \
  --packages-select serial_arm_core serial_arm_hardware_damiao serial_arm_robot_profiles dm_arm_description serial_arm_ros2_control \
  --cmake-args -DCMAKE_BUILD_TYPE=Debug
```
...
```bash
--config src/robot_supports/robots/dm_arm/description/config/core/white.yaml
```
...
### 9.8. Dynamics 与 Payload

`white.yaml` 使用带夹爪 URDF；`gray.yaml` 使用无夹爪 URDF
...
当前 CLI 仍要求提供 Hardware Backend 以取得 Capability；比较模式不执行 connect、activate 或 write
...
→ runtime.write_enabled=true 加载当前 Profile 指定的 Hardware Backend，false 使用离线 mock
→ 不连接真机
...
### 13.5. StateInterface 与 CommandInterface
...
### 13.7. JointTrajectoryController
...
Mock demo 用于验证模型、SRDF、规划组和 RViz；它不连接真实硬件

### 14.6. MoveIt 与 Core Safety
...
### 17.8. JointTrajectoryController
...
确认受控关节的 position 和 velocity 命令接口未被其他控制器占用
