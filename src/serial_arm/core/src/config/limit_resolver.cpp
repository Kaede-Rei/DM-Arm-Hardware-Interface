 ...
 */
tl::expected<ResolvedSafetyCfg, LimitResolverErr> LimitResolver::resolve(const RobotModelInfo& model, const JointActuatorMapCfg& mapper, const HardwareCapabilities& capabilities, const SafetyPolicyCfg& policy) const {
    const std::size_t n = model.joint_names.size();
    if(n == 0 || model.joint_limits.size() != n || capabilities.size() != n ||
    ...
    for(std::size_t i = 0; i < n; ++i) {
        ...
        if(capability.actuator_name.empty()) return tl::make_unexpected(LimitResolverErr::MISSING_ACTUATOR);
