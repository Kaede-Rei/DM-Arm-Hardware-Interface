...
5 先做无真机检查
    ...
    python serial_arm_terminal.py \
        --config ../../serial_arm_bringup/config/dm_arm_white.yaml \
        --check-only
...
runtime.write_enabled=true 时使用 Damiao 真机；false 时使用离线 mock 后端
    wheel 构建时启用 SERIAL_ARM_BUILD_DAMIAO
...
class Terminal:
    ...
    def run(self) -> int:
        ...
        while not self.quit:
            ...
            try:
                ...
                self.safe_exit()
            except self.dm_arm.SerialArmError as error:
    ...
    def self_check(self) -> None:
        ...
        print(
            f"serial_arm version     : {getattr(self.dm_arm, '__version__', 'unknown')}"
        )
...
def check_only(dm_arm: Any, config: Path) -> int:
    ...
    dynamics.update(zero, zero, zero, zero, zero)
    print(f"serial_arm version   : {getattr(dm_arm, '__version__', 'unknown')}")
