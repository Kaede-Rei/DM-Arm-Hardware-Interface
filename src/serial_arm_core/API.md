...
```python
session = serial_arm.RobotSession(config_file)
```
...
```cpp
const auto result = serial_arm::load_robot_cfg("dm_arm_white.yaml");
if(!result) return 1;
...
session = serial_arm.RobotSession("dm_arm_white.yaml")
