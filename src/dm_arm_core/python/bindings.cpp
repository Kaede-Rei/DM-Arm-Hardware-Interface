...
void unwrap_void(tl::expected<void, Error>&& result, MessageFn message_fn) {
    if(!result) {
        throw DmArmPythonError(message_fn(result.error()));
    }
}
...
ActuatorMitCmd mapper_to_actuator_cmd(const JointActuatorMapper& self, const JointCtrlCmd& cmd) {
...
void bind_enums(py::module_& module) {
    ...
    py::enum_<ConfigErr>(module, "ConfigErr")
        ...
        .value("DUPLICATE_MOTOR_ID", ConfigErr::DUPLICATE_MOTOR_ID);
...
void bind_state_types(py::module_& module) {
    ...
    py::class_<ActuatorMitCmd>(module, "ActuatorMitCmd")
        .def(py::init<>())
        .def_property("pos", &get_double_member<ActuatorMitCmd, &ActuatorMitCmd::pos>, &set_double_member<ActuatorMitCmd, &ActuatorMitCmd::pos>)
        .def_property("vel", &get_double_member<ActuatorMitCmd, &ActuatorMitCmd::vel>, &set_double_member<ActuatorMitCmd, &ActuatorMitCmd::vel>)
        .def_property("tor", &get_double_member<ActuatorMitCmd, &ActuatorMitCmd::tor>, &set_double_member<ActuatorMitCmd, &ActuatorMitCmd::tor>)
        .def_property("kp", &get_double_member<ActuatorMitCmd, &ActuatorMitCmd::kp>, &set_double_member<ActuatorMitCmd, &ActuatorMitCmd::kp>)
        .def_property("kd", &get_double_member<ActuatorMitCmd, &ActuatorMitCmd::kd>, &set_double_member<ActuatorMitCmd, &ActuatorMitCmd::kd>);
}
...
void bind_config(py::module_& module) {
    ...
    py::class_<DamiaoActuatorCfg>(module, "DamiaoActuatorCfg")
        ...
        .def_readwrite("joint_name", &DamiaoActuatorCfg::joint_name)
        .def_readwrite("motor_id", &DamiaoActuatorCfg::motor_id)
        .def_readwrite("master_id", &DamiaoActuatorCfg::master_id)
        .def_readwrite("motor_type", &DamiaoActuatorCfg::motor_type);

    py::class_<DamiaoBusCfg>(module, "DamiaoBusCfg")
        .def(py::init<>())
        ...
        .def_readwrite("stop_cycles", &DamiaoBusCfg::stop_cycles)
        .def_readwrite("actuators", &DamiaoBusCfg::actuators);
    ...
    py::class_<RobotCfg>(module, "RobotCfg")
        ...
        .def_readwrite("damiao", &RobotCfg::damiao)
        .def_readwrite("dynamics", &RobotCfg::dynamics);

    module.def("load_robot_cfg", [](const std::string& path) {
        return unwrap_value(load_robot_cfg(path), [](const ConfigErrInfo& error) { return error.message; });
        });
...
void bind_robot_session(py::module_& module) {
    ...
    py::class_<RobotSessionActuatorInfo>(module, "ActuatorInfo")
        ...
        .def_readonly("motor_id", &RobotSessionActuatorInfo::motor_id)
        .def_readonly("master_id", &RobotSessionActuatorInfo::master_id)
        .def_readonly("motor_type", &RobotSessionActuatorInfo::motor_type)
        .def_readonly("q_max", &RobotSessionActuatorInfo::q_max)
        .def_readonly("dq_max", &RobotSessionActuatorInfo::dq_max)
        .def_readonly("tau_max", &RobotSessionActuatorInfo::tau_max);
...
PYBIND11_MODULE(_dm_arm, module) {
    ...
    py::register_exception<dm_arm::DmArmPythonError>(module, "DmArmError");

    dm_arm::python_binding::bind_enums(module);
    dm_arm::python_binding::bind_state_types(module);
    dm_arm::python_binding::bind_config(module);
    dm_arm::python_binding::bind_core(module);
    dm_arm::python_binding::bind_dynamics(module);
    dm_arm::python_binding::bind_robot_session(module);
}
