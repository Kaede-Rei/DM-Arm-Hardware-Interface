 ...
 */
void DmMotorBus::activate(int startup_read_cycles, impedance::JointState& state) {
 ...
 */
bool DmMotorBus::read(bool refresh_state, impedance::JointState& state) noexcept {
    try {
 ...
 * @return 成功返回 true，失败返回 false
 */
bool DmMotorBus::write(std::size_t index, const impedance::MitJointCommand& command) noexcept {
    try {
 ...
 * @return 成功返回 true，失败返回 false
 */
bool DmMotorBus::read_one(std::size_t index, bool refresh_state, impedance::JointState& state) noexcept {
    try {
