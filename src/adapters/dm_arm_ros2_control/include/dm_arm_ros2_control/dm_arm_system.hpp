...
#include "dm_arm_ros2_control/status_types.hpp"
 ...
 */
class DmArmSystem final : public hardware_interface::SystemInterface {
public:
    DmArmSystem() = default;
    /**
     * @brief 停止后台线程并尽力释放真机硬件
     */
    ~DmArmSystem() override;
...
private:
     ...
     * @return 模型前馈回调函数
     */
    dm_arm::ModelFeedforwardFn make_model_feedforward();
    /**
     ...
     */
    std::string make_robot_error(const char* action, const dm_arm::RobotFault& fault) const;

private:
    dm_arm::RobotCfg cfg_;                         ///< Core 完整配置
    std::unique_ptr<dm_arm::Robot> robot_;         ///< 后台线程独占的 Robot 实例
